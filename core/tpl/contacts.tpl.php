...
if ($permission) {
	...
	<form class="tagtr liste_titre">
		...
		<div class="tagtd liste_titre"><?php echo $langs->trans("Ref"); ?></div>
	...
	<form class="tagtr impair nohover" action="<?php echo $_SERVER["PHP_SELF"].'?id='.$object->id; ?>" method="POST">
        ...
        $tmpobject->element = "kjraffaire";
		$formcompany->selectTypeContact($tmpobject, '', 'type', 'internal', 'position', 0, 'minwidth150imp widthcentpercentminusx maxwidth200'); ?></div>
		<!-- Nouvelle colonne pour le champ Réf. -->
		<div class="tagtd maxwidthonsmartphone">
			<input type="text" name="reference" class="flat minwidth150imp" placeholder="<?php echo $langs->trans("EnterReference"); ?>" value="<?php echo dol_escape_htmltag(GETPOST('reference', 'alpha')); ?>">
		</div>
		<div class="tagtd">&nbsp;</div>
	...
	if (empty($hideaddcontactforthirdparty)) {
        // Pré-charger tous les sous-types pour tous les types parents
        $allSubtypes = [];
        $sql = "SELECT fk_type_contact, rowid, libelle 
                FROM ".MAIN_DB_PREFIX."kjraffaire_dico_soustype_contact 
                WHERE active = 1 
                ORDER BY fk_type_contact, position ASC";
        $resql = $db->query($sql);
        
        if ($resql) {
            while ($obj = $db->fetch_object($resql)) {
                $allSubtypes[$obj->fk_type_contact][] = [
                    'rowid' => $obj->rowid,
                    'libelle' => $obj->libelle,
                ];
            }
	...
	<form class="tagtr pair nohover" action="<?php echo $_SERVER["PHP_SELF"].'?id='.$object->id; ?>" method="POST">
		...
		<div class="tagtd noborderbottom">
            ...
            <select id="subtype_contact" name="subtype_contact" class="flat" style="display: none;">
                <option value="">-- Sélectionner un sous-type --</option>
                <?php
                // Charger toutes les options pour le deuxième select dans un data-attribute
                foreach ($allSubtypes as $fk_type_contact => $subtypes) {
                    foreach ($subtypes as $subtype) {
                        echo '<option value="'.$subtype['rowid'].'" data-parent="'.$fk_type_contact.'" style="display: none;">'.$subtype['libelle'].'</option>';
                    }
                }
                ?>
            </select>
		</div>
		<!-- Colonne pour le champ Réf. -->
		<div class="tagtd maxwidthonsmartphone">
			<input type="text" name="reference" class="flat minwidth150imp" placeholder="<?php echo $langs->trans("EnterReference"); ?>" value="<?php echo dol_escape_htmltag(GETPOST('reference', 'alpha')); ?>">
		</div>
		<div class="tagtd noborderbottom">&nbsp;</div>
		<div class="tagtd center noborderbottom">
			<input type="submit" id="add-customer-contact" class="button small" value="<?php echo $langs->trans("Add"); ?>"<?php if (!$nbofcontacts) { echo ' disabled'; } ?>>
		</div>
	</form>
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            const typeContactSelect = document.querySelector('select[name="typecontact"]');
            const subtypeContactSelect = document.querySelector('#subtype_contact');

            // Fonction pour mettre à jour le deuxième select
            function updateSubtypeSelect(selectedValue) {
                // Réinitialise le second select
                Array.from(subtypeContactSelect.options).forEach(option => {
                    if (option.dataset.parent === selectedValue) {
                ...
                // Affiche ou masque le second select
                if (Array.from(subtypeContactSelect.options).some(option => option.style.display === 'block')) {
                    subtypeContactSelect.style.display = 'block';
                } else {
                    subtypeContactSelect.style.display = 'none';
                }
            }

            // Ajoute un événement spécifique à Select2
            $(typeContactSelect).on('change.select2', function () {
                updateSubtypeSelect(this.value);
            });

            // Appel initial pour afficher correctement le deuxième select au chargement
            if (typeContactSelect.value) {
                updateSubtypeSelect(typeContactSelect.value);
            }
        });
    </script>
...
print_liste_field_titre($langs->trans("Ref."), $_SERVER["PHP_SELF"], "reference", "", $param, "", $sortfield, $sortorder, 'center'); // Ajout de l’en-tête pour "Ref."
...
$subtypeLabels = [];
$sql = "SELECT ec.rowid AS id_element_contact, 
               dic.libelle AS subtype_label, 
               ref.reference AS reference_label
        FROM ".MAIN_DB_PREFIX."element_contact AS ec
        LEFT JOIN ".MAIN_DB_PREFIX."kjraffaire_souselement_contact AS sec ON ec.rowid = sec.id_element_contact
        LEFT JOIN ".MAIN_DB_PREFIX."kjraffaire_dico_soustype_contact AS dic ON sec.id_soustype_contact = dic.rowid
        LEFT JOIN ".MAIN_DB_PREFIX."kjraffaire_souselement_reference AS ref ON ec.rowid = ref.id_element_contact";

$resql = $db->query($sql);
if ($resql) {
    while ($obj = $db->fetch_object($resql)) {
        $elementData[$obj->id_element_contact] = [
            'subtype_label' => $obj->subtype_label,
            'reference_label' => $obj->reference_label,
        ];
    }
} else {
...
foreach ($list as $entry) {
    ...
    // Affichage du rôle avec le sous-type s'il existe
    $subtypeLabel = isset($elementData[$entry->id]['subtype_label']) ? ' > ' . dol_escape_htmltag($elementData[$entry->id]['subtype_label']) : '';
    print '<td class="tdoverflowmax200" data-type_id="' . ((int) $entry->type_id) . '" data-type="' . dol_escape_htmltag($entry->type) . '">'.dol_escape_htmltag($entry->type) . $subtypeLabel.'</td>';

    // Affichage de la colonne "Ref."
    $referenceLabel = isset($elementData[$entry->id]['reference_label']) ? dol_escape_htmltag($elementData[$entry->id]['reference_label']) : '';
    print '<td class="tdoverflowmax200">'.$referenceLabel.'</td>'; // Colonne "Ref."
