...
if (!isset($existingFields['fk_kjraffaire_dico_juridiction'])) {
    $res = $extrafields->addExtraField( 'fk_kjraffaire_dico_juridiction','Juridiction','int',102,'','projet', 0, 0, '', '', 1, '','($object->array_options[\'options_affaire\']==1)?1:0' );
...
if (!isset($existingFields['fk_kjraffaire_dico_action_juridique'])) {
    $res = $extrafields->addExtraField( 'fk_kjraffaire_dico_action_juridique','Action juridique','int',103,'','projet', 0, 0, '', '', 1, '','($object->array_options[\'options_affaire\']==1)?1:0' );
...
if (!isset($existingFields['chambre'])) {
    ...
    $res = $extrafields->addExtraField(
        'chambre','Chambre','varchar',104,30,'projet',0,0,'', $params,1,'','($object->array_options[\'options_affaire\']==1)?1:0'
    );
    if ($res < 0) {
        // Gestion des erreurs
...
if (!isset($existingFields['no_role'])) {
    ...
    $res = $extrafields->addExtraField(
        'no_role','No role','varchar',105,30,'projet',0,0,'', $params,1,'','($object->array_options[\'options_affaire\']==1)?1:0'
    );
    if ($res < 0) {
        // Gestion des erreurs
...
if (!isset($existingFields['fk_soc_magistrat'])) {
    $res = $extrafields->addExtraField( 'fk_soc_magistrat','Magistrat','int',106,'','projet', 0, 0, '', '', 1, '','($object->array_options[\'options_affaire\']==1)?1:0' );
...
if (!isset($existingFields['fk_socpeople_magistrat'])) {
    $res = $extrafields->addExtraField( 'fk_socpeople_magistrat','Magistrat','int',107,'','projet', 0, 0, '', '', 1, '','($object->array_options[\'options_affaire\']==1)?1:0' );
...
if (!isset($existingFields['section'])) {
    ...
    $res = $extrafields->addExtraField(
        'section','Section','varchar',108,30,'projet',0,0,'', $params,1,'','($object->array_options[\'options_affaire\']==1)?1:0'
    );
    if ($res < 0) {
        // Gestion des erreurs
...
if (!isset($existingFields['date_decision'])) {
    $res = $extrafields->addExtraField('date_decision','Date décision','date',109,'','projet',0,0,'',
        array('options' => array()),
        1,'','($object->array_options[\'options_affaire\']==1)?1:0');
...
if (!isset($existingFields['fk_soc_avocat_postulant'])) {
    $res = $extrafields->addExtraField( 'fk_soc_avocat_postulant','Avocat postulant','int',112,'','projet', 0, 0, '', '', 1, '','($object->array_options[\'options_affaire\']==1)?1:0' );
...
if (!isset($existingFields['fk_socpeople_avocat_postulant'])) {
    $res = $extrafields->addExtraField( 'fk_socpeople_avocat_postulant','Avocat postulant','int',113,'','projet', 0, 0, '', '', 1, '','($object->array_options[\'options_affaire\']==1)?1:0' );
