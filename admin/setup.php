...
/**
 * \file    kjraffaire/admin/setup.php
 * \ingroup kjraffaire
 * \brief   Kjraffaire setup page.
 */
...
require_once '../lib/kjraffaire.lib.php';
//require_once "../class/myclass.class.php";

// Translations
$langs->loadLangs(array("admin", "kjraffaire@kjraffaire"));

// Initialize technical object to manage hooks of page. Note that conf->hooks_modules contains array of hook context
$hookmanager->initHooks(array('kjraffairesetup', 'globalsetup'));
...
if ($action == 'updateMask') {
	...
	foreach ($dirmodels as $reldir) {
		$file = dol_buildpath($reldir."core/modules/kjraffaire/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file)) {
			$filefound = 1;
	...
	if ($filefound) {
		...
		if ($module->write_file($tmpobject, $langs) > 0) {
			header("Location: ".DOL_URL_ROOT."/document.php?modulepart=kjraffaire-".strtolower($tmpobjectkey)."&file=SPECIMEN.pdf");
			return;
		} else {
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'KJRAFFAIRE_'.strtoupper($tmpobjectkey)."_ADDON";
	...
	if ($ret > 0) {
		if (!empty($tmpobjectkey)) {
			$constforval = 'KJRAFFAIRE_'.strtoupper($tmpobjectkey).'_ADDON_PDF';
...
$help_url = '';
$title = "KjraffaireSetup";

llxHeader('', $langs->trans($title), $help_url, '', 0, 0, '', '', '', 'mod-kjraffaire page-admin');
...
$head = kjraffaireAdminPrepareHead();
print dol_get_fiche_head($head, 'settings', $langs->trans($title), -1, "kjraffaire@kjraffaire");

// Setup page goes here
echo '<span class="opacitymedium">'.$langs->trans("KjraffaireSetupPage").'</span><br><br>';
					...
					while (($file = readdir($handle)) !== false) {
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php') {
							...
							if ($module->isEnabled()) {
								...
								$constforvar = 'KJRAFFAIRE_'.strtoupper($myTmpObjectKey).'_ADDON';
							...
							if (preg_match('/\.modules\.php$/i', $file) && preg_match('/^(pdf_|doc_)/', $file)) {
								if (file_exists($dir.'/'.$file)) {
									...
									if ($modulequalified) {
										...
										// Default
										print '<td class="center">';
										$constforvar = 'KJRAFFAIRE_'.strtoupper($myTmpObjectKey).'_ADDON_PDF';
