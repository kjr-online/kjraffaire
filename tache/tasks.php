...
$title = 'Affaire - '.$object->ref.' '.$object->name;
...
if ($id > 0 || !empty($ref)) {
	...
	print dol_get_fiche_head($head, $tab, $langs->trans("Project"), -1, ($object->public ? 'fa-briefcase' : 'fa-briefcase'));
...
if ($action == 'create' && $user->hasRight('projet', 'creer') && (empty($object->thirdparty->id) || $userWrite > 0)) {
	...
	print load_fiche_titre($langs->trans("NewTask"), '', 'fa-briefcase');
	...
	$linktocreatetask = dolGetButtonTitle($langs->trans('AddTask'), '', 'fa fa-plus-circle', DOL_URL_ROOT.'/custom/kjraffaire/tache/tasks.php?action=create'.$param.'&backtopage='.urlencode($_SERVER['PHP_SELF'].'?id='.$object->id), '', $linktocreatetaskUserRight, $linktocreatetaskParam);
	...
	$linktotasks = dolGetButtonTitle($langs->trans('ViewList'), '', 'fa fa-bars imgforviewmode', DOL_URL_ROOT.'/custom/kjraffaire/tache/tasks.php?id='.$object->id, '', 1, array('morecss' => 'reposition btnTitleSelected'));
	...
	*/
	...
	if (count($tasksarray) > 0) {
		...
		$j = 0;
		$level = 0;
		$nboftaskshown = affaireLinesa($j, 0, $tasksarray, $level, true, 0, $tasksrole, $object->id, 1, $object->id, '', ($object->usage_bill_time ? 1 : 0), $arrayfields, $arrayofselected);
	} else {
