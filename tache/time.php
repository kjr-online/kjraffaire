...
if (($id > 0 || !empty($ref)) || $projectidforalltimes > 0 || $allprojectforuser > 0) {
	...
	if ($projectstatic->id > 0) {
		if ($withproject) {
			...
			$head = affaire_prepare_head($projectstatic);
			print dol_get_fiche_head($head, $tab, $langs->trans("Project"), -1, ($projectstatic->public ? 'fa-briefcase' : 'fa-briefcase'));
			...
			if ($projectstatic->public == 1) {
				print img_picto($langs->trans('SharedProject'), 'world', 'class="paddingrightonly"');
				print $langs->trans('SharedProject');
			} elseif ($projectstatic->public == 0) {
				print img_picto($langs->trans('PrivateProject'), 'private', 'class="paddingrightonly"');
				print $langs->trans('PrivateProject');
			} elseif ($projectstatic->public == 2) {
				...
				$group_names = [];
				$sql = "SELECT group_id FROM ".MAIN_DB_PREFIX."kjraffaire_visibility_group WHERE affaire_id = ".$db->escape($projectstatic->id);
				$resql = $db->query($sql);
				if ($resql) {
