	...
	public function getHtmlForTo()
	{
		...
		if ($this->withtoreadonly) {
			...
			if (!empty($this->withto) && is_array($this->withto)) {
				...
				// NOTE BP:
				// Récupérer les éléments de 'kjraffaire' depuis la base de données
				$sql = "SELECT ec.rowid, sp.lastname, sp.firstname, sp.email 
				...
				if ($resql) {
					while ($obj = $this->db->fetch_object($resql)) {
						...
						// Créer un label unique pour chaque contact
						$uniqueLabel = $obj->firstname . ' ' . $obj->lastname . ' (' . $obj->email . ')';
			
						// Vérifier que l'entrée n'existe pas déjà dans $tmparray
						$isDuplicate = false;
						foreach ($tmparray as $key => $entry) {
							if (isset($entry['label']) && $entry['label'] === $uniqueLabel) {
								$isDuplicate = true;
								break;
							}
						}
                ...
                // Réorganiser pour mettre 'thirdparty' à la fin
                $thirdpartyEntries = [];
                foreach ($tmparray as $key => $entry) {
                    if ($entry['id'] === 'thirdparty') {
                        $thirdpartyEntries[$key] = $entry;
                        unset($tmparray[$key]);
                    }
                }
                $tmparray = array_merge($tmparray, $thirdpartyEntries);
				// TODO BP : Ajouter les valeurs à ce tableau
				$uniqueArray = [];
				foreach ($tmparray as $key => $val) {
					// Extraire l'email du label
					if (preg_match('/\(([^)]+)\)/', $val['label'], $matches)) {
						$email = $matches[1]; // Email extrait
						if (!isset($uniqueArray[$email])) {
							// Ajouter l'entrée si l'email n'est pas encore dans le tableau unique
							$uniqueArray[$email] = $val;
						}
					} else {
						// Si pas d'email, ajouter directement
						$uniqueArray[$key] = $val;
					}
				}

				// Réassigner le tableau sans doublons
				$tmparray = array_values($uniqueArray);
